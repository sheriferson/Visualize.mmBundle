...
function pieByTag(emails) {

    // aggregate the count of emails per tag
    var data = d3.nest()
    ...
    var svg = d3.select("#pieByTag").append("svg")
      ...
      // we will use this value to only automatically show names of tags
      // that have frequencies above a certain threshold
      ...
      var tooltip = d3.select('#pieByTag').append('div')
...
function barByTag(emails) {
    ...
    // rename variables in the aggregated set
    ...
    // limit number of tags represented in bar graph to 50
    var originalTagNumber = data.length
    if (originalTagNumber > 50)
    {
      data = data.slice(0, 50)
      var excludedTagNumber = originalTagNumber - data.length

      if (excludedTagNumber < 2) {
        var exTagMessage = " tags couldn't fit in the bar graph and was excluded."
      } else {
        exTagMessage = " tags couldn't fit in the bar graph and were excluded."
      }

      d3.select('#excludedTag')
        .style('fill', '#000')
        .style('color', '#c43c35')
    ...
    // plot the data into an svg
   d3.selectAll('#barByTag').append('svg')
        .attr('id', 'barchart')
    ...
    var tooltip = d3.select('#barByTags').append('div')
        .style('position', 'absolute')
        .style('background', 'lightblue')
        .style('color', 'black')
        .style('padding', '0 10px')
...
d3.select("#savePie").on("click", function(){
  saveSvgAsPng(document.getElementById("pie"), "pie_by_tag.png"); 
});

d3.select("#saveBar").on("click", function(){
  saveSvgAsPng(document.getElementById("barchart"), "bar_by_tag.png"); 
});
